...
def make_dataframe(SubIDs,Categories,Timepoints):    
    Y = pd.DataFrame([SubIDs,Categories,Timepoints])
    Y = Y.transpose()
    Y.columns = ['subid','category','timepoint']   
    return Y
...
def preprocess_features(Features, Y):
    _Y = Y.sort_values(['subid','category','timepoint'])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, help='dataset rendered version, e.g., prakash', default="prakash")    

    args = parser.parse_args()
    
    ## get list of all sketch paths
    sketch_paths = sorted(list_files(args.data,args.ext))
    print('Length of sketch_paths: {}'.format(len(sketch_paths)))
    ...
    Y = make_dataframe(SubIDs, Categories, Timepoints)
