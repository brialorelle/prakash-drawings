...
class FeatureExtractor():
    ...
    def extract_feature_matrix(self):
        ...
        def get_metadata_from_path(path,dataset):
            ...
            if self.dataset=='prakash':
                subid = path.split('/')[-1].split('_')[0]
                category = path.split('/')[-1].split('_')[3]
                timepoint_temp = path.split('/')[-1].split('_')[4]
                timepoint = timepoint_temp.split('.png')[0]
            else:
                print('Unspecified dataset!')
                subid = 'unknown'
                category = 'unknown'
                timepoint = 'unknown'
            return subid, category, timepoint

        def generator(paths, imsize=self.imsize, use_cuda=use_cuda, dataset=self.dataset):
            for path in paths:
                image = load_image(path)
                subid, category, timepoint = get_metadata_from_path(path,dataset)
                yield (image, subid, category, timepoint)   
        ...
        if generator:
            while True:    
                ...
                if n == self.num_sketches//self.batch_size:
                    ...
                    subid_batch = subid_batch[:b + 1] 
                    category_batch = category_batch[:b + 1]   
                    timepoint_batch = timepoint_batch[:b + 1]
